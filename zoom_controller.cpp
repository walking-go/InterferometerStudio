...
ZoomController::ZoomController(QObject *parent) : QObject(parent)
{
    ...
    // 连接串口接收信号（解析控制盒返回数据）
    connect(m_serial, &QSerialPort::readyRead, this, &ZoomController::readSerialData);
    // 连接串口错误信号
...
// 串口配置：设置串口名
void ZoomController::setSerialPort(const QString &portName)
{
    m_serial->setPortName(portName);
}

// 打开串口
bool ZoomController::openSerial()
{
...
// 关闭串口
void ZoomController::closeSerial()
{
    if (m_serial->isOpen()) {
        m_serial->close();
        emit serialStatusChanged("未连接");
...
// 判断串口是否打开
bool ZoomController::isSerialOpen() const
{
    return m_serial->isOpen();
}

// 接收界面信号：执行固定倍率切换（文档定倍指令）
void ZoomController::FixedZoom(quint8 cmd)
{
    if (!m_serial->isOpen()) {
...
void ZoomController::StartZoom(quint8 cmd)
{
    if (!m_serial->isOpen()) {
...
// 接收界面信号：开始微调（文档微调指令）
void ZoomController::StartFineTune(quint8 cmd)
{
    if (!m_serial->isOpen()) {
...
void ZoomController::StopFineTune()
{
    m_fineTuneTimer->stop();
    sendControlCmd(0x0A); // 发送停止指令
}
...
void ZoomController::readSerialData()
{
...
void ZoomController::sendControlCmd(quint8 cmd, QByteArray params)
{
    QByteArray cmdData;
...
QString ZoomController::positionToZoom(quint32 position)
{
